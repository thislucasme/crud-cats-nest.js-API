...
export class CatsController {
	...
	async create(@Body() createCatDto: CreateCatDto, @Res() res: Response){
		const result = await this.catsService.criar(createCatDto).then(() => {
			res.status(HttpStatus.CREATED).json({status: HttpStatus.CREATED, info: "Gato criado com sucesso", createCatDto})
		}).catch(error => {
			res.status(HttpStatus.BAD_REQUEST).json({ info: "Erro ao salvar gato", status: HttpStatus.BAD_REQUEST });
		});
		console.log(result);
	}
	@Get(':id')
