...
export class CatsService {
	...
	async getGato(nome: string): Promise<Cat | undefined> {
	...
	async findOne(email: string): Promise<Cat | undefined> {
		return await this.databaseService.bd().select().where('email', '=', email).from(this.tableName).first();
	}
	async findCatById(id: number): Promise<Cat | undefined> {
		return await this.databaseService.bd().select().where('idGato', '=', id).from(this.tableName).first();
	}
}
